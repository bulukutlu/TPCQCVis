...
for run in runList:
    target = subprocess.run(["alien.py","find",directory+run+"/apass1/QC/001/","QC.root"], capture_output=True)
    if len(target.stdout) > 0 : 
        ...
        target = subprocess.run(["alien.py","find",directory+run+"/apass1/","QC_fullrun.root"], capture_output=True)
        if len(target.stdout) > 0 : 
            target_path = target.stdout[:-1].decode('UTF-8')
            print("Downloading "+target_path)
            subprocess.run(["alien.py","cp","alien:"+target_path,"file:"+local_dir+run+".root"])
        else: 
            print("File "+directory+run+"/apass1/QC/001/QC.root"+ "  not found!")